      ...
      <form method="post" action="<?=$apaymentin?>">
        <table width="100%">
          <tr>
            ...
            <td class="align-center"><input style="width:200px;" type="text" name="amount" value="<?=$settings['paypal_minimalni_castka'];?>" /></td> 
          </tr>
          <tr>       
...
<?}else{?>
  <?=frontendMessage('normal',$systemTranslator['paypal_neovereny_email_nejde_delat_nic']);?>
<?}?>
<?if(count($coins)>0){?> 
  <?if(getget('message')=='refresh-success'){?><?=frontendMessage('green',$systemTranslator['paypal_refresh_success']);?><?}?> 
      ...
      <tr>      
        ...
        <td>
          ...
          <?}else{?>          
            ...
            <?if($cs->type=='pay-out'&&1==2){?>
              <a href="<?=$cs->aRefreshOut?>"><em class="fa fa-refresh "></em></a>
            <?}?>
          <?}?>
        </td>
      </tr>
