...
class Module{
  ...
  public function getUserRights(){
  	$rights=new stdClass();
  	$rights->prava=$this->kernel->user->data->prava;
  	$rights->dbRights=array(); 
  	$rights->povoleneKody=array(); 
  	$rights->povoleneModuly=array(); 
  	$rights->povolenePodmoduly=array();  	
  	if($this->kernel->user->data->prava==1){ // omezena prava
  		$rights->dbRights=$this->kernel->models->DBusers->MqueryGetLines('SELECT ar.* FROM administrators_rights as ar,administrators_rights_users as aru WHERE ar.idar=aru.idar AND aru.idu="'.((int)$this->kernel->user->data->uid).'"');	
	  }elseif($this->kernel->user->data->prava==2){ // neomezena prava
	  	$rights->dbRights=$this->kernel->models->DBusers->MqueryGetLines('SELECT ar.* FROM administrators_rights as ar');
	  }else{ // zadna prava
	  	return $rights;	
	  }
	  foreach($rights->dbRights as $dbr){
	  	$rights->povoleneKody[$dbr->kod]=$dbr->kod; 
			$rights->povoleneModuly[$dbr->povoluje_modul]=$dbr->povoluje_modul; 
			$rights->povolenePodmoduly[$dbr->povoluje_podmodul]=$dbr->povoluje_podmodul; 
	  	}	  
  	return $rights;
  	}
  }
