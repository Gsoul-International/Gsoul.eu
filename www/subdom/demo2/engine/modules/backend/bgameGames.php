...
class BGameGames extends Module{
  ...
  public function PageGamesAddParameterPost(){
    ...
    $this->kernel->models->DBgamesParameters->store(0,array('nazev'=>$nazev,'idg'=>$idg,'poradi'=>$poradi,'typ_v_turnaji_cupu'=>$typ_v_turnaji_cupu,'typ_v_tabulce'=>$typ_v_tabulce,'zobrazovat_v_tabulce'=>$zobrazovat_v_tabulce));      
    $this->redirect(array('module'=>$this->parent_module,'action'=>'games-edit','message'=>'created','idg'=>$idg,'bp'=>$bp),false);
    }
  public function PageGamesEditParameterPost(){
    ...
    $zobrazovat_v_tabulce=(int)getpost('zobrazovat_v_tabulce','');   
    $poradi=(int)getpost('poradi','');     
    if($idp>0){$this->kernel->models->DBgamesParameters->store($idp,array('nazev'=>$nazev,'typ_v_turnaji_cupu'=>$typ_v_turnaji_cupu,'typ_v_tabulce'=>$typ_v_tabulce,'zobrazovat_v_tabulce'=>$zobrazovat_v_tabulce,'poradi'=>$poradi));}      
    $this->redirect(array('module'=>$this->parent_module,'action'=>'games-edit','message'=>'edited','idg'=>$idg,'bp'=>$bp),false);
    }
  public function PageGamesAddSubParameterPost(){
    ...
    $poradi=(int)getpost('poradi','');   
    if($idp>0){$this->kernel->models->DBgamesParametersValues->store(0,array('nazev'=>$nazev,'idp'=>$idp,'poradi'=>$poradi,'aktivni'=>$aktivni));}      
    $this->redirect(array('module'=>$this->parent_module,'action'=>'games-edit','message'=>'sub-created','idg'=>$idg,'bp'=>$bp),false);  
    }
  public function PageGamesEditSubParameterPost(){
    ...
    $aktivni=(int)getpost('aktivni','');   
    $poradi=(int)getpost('poradi','');   
    if($idpv>0){$this->kernel->models->DBgamesParametersValues->store($idpv,array('nazev'=>$nazev,'poradi'=>$poradi,'aktivni'=>$aktivni));}      
    $this->redirect(array('module'=>$this->parent_module,'action'=>'games-edit','message'=>'sub-edited','idg'=>$idg,'bp'=>$bp),false);  
    }
  public function PageAddPost(){
    ...
    $this->kernel->models->DBrewrites->AddEditRewrite('tournaments/'.$nazev.'-'.$idg.'/','FTournaments','idg',$idg);   
    $this->kernel->models->DBrewrites->AddEditRewrite('cups/'.$nazev.'-'.$idg.'/','FCups','idg',$idg);
  ...
  public function PageEditPost(){
    ...
    $this->kernel->models->DBgames->store($idg,array('nazev'=>$nazev,'mail_ukonceni_zapasu'=>$mail_ukonceni_zapasu,'aktivni'=>$aktivni,'pravidla_turnaje'=>$pravidla_turnaje,'podrobna_pravidla_turnaje'=>$podrobna_pravidla_turnaje, 'zobraz_buyin'=>$zobraz_buyin, 'zobraz_typhry'=>$zobraz_typhry, 'zobraz_pocethracu'=>$zobraz_pocethracu, 'zobraz_datumzahajeni'=>$zobraz_datumzahajeni, 'zobraz_pravidla'=>$zobraz_pravidla,'zobrazit_dohrano'=>$zobrazit_dohrano,'zobraz_login'=>$zobraz_login ,'zaklada_jen_admin'=>$zaklada_jen_admin));
  ...
  public function PageAddTypePost(){
    ...
    $this->kernel->models->DBgamesTypes->store(0,array('nazev'=>$nazev,'idg'=>$idg,'aktivni'=>$aktivni,'tournament_id_vyplaty'=>$tournament_id_vyplaty,'tournament_minimalni_pocet_hracu'=>$tournament_minimalni_pocet_hracu,'tournament_maximalni_pocet_hracu'=>$tournament_maximalni_pocet_hracu,'tournament_minimalni_pocet_tymu'=>$tournament_minimalni_pocet_tymu,'tournament_maximalni_pocet_tymu'=>$tournament_maximalni_pocet_tymu,'cup_id_vyplaty'=>$cup_id_vyplaty,'cup_minimalni_pocet_hracutymu'=>$cup_minimalni_pocet_hracutymu,'cup_maximalni_pocet_hracutymu'=>$cup_maximalni_pocet_hracutymu,'cup_pocet_postupujicich_hracutymu'=>$cup_pocet_postupujicich_hracutymu,'cup_idealni_pocet_hracu_v_tymu'=>$cup_idealni_pocet_hracu_v_tymu,'cup_idealni_pocet_hracutymu_na_turnaj'=>$cup_idealni_pocet_hracutymu_na_turnaj));      
  ...
  public function PageEditTypePost(){
    ...
    $this->kernel->models->DBgamesTypes->store($idgs,array('nazev'=>$nazev,'aktivni'=>$aktivni,'tournament_id_vyplaty'=>$tournament_id_vyplaty,'tournament_minimalni_pocet_hracu'=>$tournament_minimalni_pocet_hracu,'tournament_maximalni_pocet_hracu'=>$tournament_maximalni_pocet_hracu,'tournament_minimalni_pocet_tymu'=>$tournament_minimalni_pocet_tymu,'tournament_maximalni_pocet_tymu'=>$tournament_maximalni_pocet_tymu,'cup_id_vyplaty'=>$cup_id_vyplaty,'cup_minimalni_pocet_hracutymu'=>$cup_minimalni_pocet_hracutymu,'cup_maximalni_pocet_hracutymu'=>$cup_maximalni_pocet_hracutymu,'cup_pocet_postupujicich_hracutymu'=>$cup_pocet_postupujicich_hracutymu,'cup_idealni_pocet_hracu_v_tymu'=>$cup_idealni_pocet_hracu_v_tymu,'cup_idealni_pocet_hracutymu_na_turnaj'=>$cup_idealni_pocet_hracutymu_na_turnaj));      
