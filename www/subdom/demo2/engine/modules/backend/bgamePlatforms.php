...
class BGamePlatforms extends Module{
  ...
  public function PageEdit(){
    ...
    foreach($dataGamesPlatforms2 as $dp2){$dataGamesPlatforms[]=$dp2->idg;}
    if(!isset($data->idgp)||$data->idgp<1){$this->redirect(array('module'=>$this->parent_module,'action'=>'platforms','message'=>'not-found'),false);}    
    $return=new stdClass();     
    $tpl=new Templater();
    $tpl->add('data',$data);
  ...
  public function PageAddPost(){
    ...
    if($nazev==''){$this->redirect(array('module'=>$this->parent_module,'action'=>'platforms','message'=>'not-created'),false);}
  ...
  public function PageEditPost(){
    ...
    $nazev=prepare_get_data_safely(getpost('nazev',''));    
    $aktivni=(int)getpost('aktivni','');    
    if($nazev==''){$this->redirect(array('module'=>$this->parent_module,'action'=>'platform-edit','message'=>'not-saved','idgp'=>$idgp),false);}
    ...
    if(isset($_POST['hry'])&&count($_POST['hry'])>0){foreach($_POST['hry'] as $xhp){
  ...
  public function PageDeletePost(){
    $idgp=(int)getget('idgp','');
    if($idgp<1){$this->redirect(array('module'=>$this->parent_module,'action'=>'platforms','message'=>'not-found'),false);}
