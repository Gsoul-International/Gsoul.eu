...
<div class="BackendBack">
  ...
  <a href="<?=$afrontend;?>" target="_blank">Zobrazit stránku na webu <i class="fa fa-arrow-right"></i> </a><br>  
</div>
<?if(getget('message','')=='name-required'){?><h2>Název stránky je povinný! Změny nebyly uloženy.</h2><?}?>
<?if(getget('message','')=='page-edited'){?><h2>Stránka uložena.</h2><?}?>
<form action="<?=$aedit;?>" method="post" autocomplete="off">
  ...
  <table>        
    <tr>
      <th colspan="2">Nadřazená stránka (změnou pozice přesunete i případné podstránky)</th>
    </tr>
    <tr>
      <td colspan="2">
    ...
    <tr style="display:none;">
      <th colspan="2">Typ obrázkového menu pro podstránky</th>
    </tr>
    <tr style="display:none;">
      <td colspan="2">
        <select name="typ_podmenu" class="width-740">
          <option value="0" <?if($page->typ_podmenu==0){?>selected<?}?> >Typ A - malý obrázek vlevo</option>
          <option value="1" <?if($page->typ_podmenu==1){?>selected<?}?> >Typ B - malý obrázek nad</option>  
          <option value="2" <?if($page->typ_podmenu==2){?>selected<?}?> >Typ C - větší obrázek v pozadí</option>  
          <option value="3" <?if($page->typ_podmenu==3){?>selected<?}?> >Typ D - větší obrázek nad</option>       
        </select>
     </td>      
    </tr>  
    <tr>
    ...
    <tr>
      ...
      <td align="right"><input type="submit" value="Uložit změny na stránce" /></td>
    </tr>
  </table>
  <br />
  <table>
    <tr>
      <th>Drobečková navigace stránky na&nbsp;webu</th>
      <td>
        <?if($page->zobrazovat_navigaci==0){?>
          <b>Navigace u této stránky není povolena, na webu se nevypíše.</b><br /> <br /> 
        <?}?>
    ...
    <tr>
      <th>Úplná drobečková navigace stránky včetně skrytých položek</th>
      <td>
        <?foreach($pageBreadcrumb as $pb){?>
    ...
    <tr>
      <th>Obecná adresa stránky</th>
      <td><?=$afrontend;?></td>
    </tr>    
    <tr>
      <th>Úplná obecná adresa stránky</th>
