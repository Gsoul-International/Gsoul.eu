...
<div class="BackendBack">
  &nbsp;<br>
  <?if(in_array('users_creates_edit',$thisUserRights->povoleneKody)){?><a href="<?=$anew;?>"><i class="fa fa-user-plus"></i> Přidat uživatele</a><?}?><br>  
</div>
	...
	<table>
	  ...
	  <?}else{?>
		<tr>
		  ...
		  <th>Uživatel</th>
		  <th>Nickname</th>
		  <th><?if(in_array('users_payments_views',$thisUserRights->povoleneKody)){?>$<?}?></th>
		  <th>Práva</th>
		...
		<?foreach($users as $u){?>
		  <tr>
			...
			<td><?if(in_array('users_payments_views',$thisUserRights->povoleneKody)){?><b><?=printcost($u->ucetni_zustatek);?></b><?}?></td>
			<td>
			  <?
			  if($u->prava==0){
				echo 'Uživatel';
			...
			<td><?if(in_array('users_payments_views',$thisUserRights->povoleneKody)){?><a title="Účetní zůstatek uživatele" href="<?=$u->acoins;?>"><i class="fa fa-usd"></i></a><?}?></td>
		  </tr>
		<?}?>  
		<tr> 
