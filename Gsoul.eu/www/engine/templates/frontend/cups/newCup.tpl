...
<h1><?=$systemTranslator['cups_cups'];?> - <?=$systemTranslator['cups_pridani_noveho_cupu_pro_hru'];?> 
<?=$game->nazev?></h1>
...
<form method="post" action="<?=$anext?>">
            ...
            <tr>
              ...
              <td>
                <label style="display:contents;font-size:1em;cursor:pointer;">
                  ...
                  <input type="radio" name="idgt" value="<?=$g->idgt?>" <?if($g->idgt==$ngData->idgt){?>checked<?}?> 
                  required />
  ...
  <div class="col col-1-3"><label for="hraji_tymy"><?=$systemTranslator['turnaje_hra_na_tymy'];?></label><select name="hraji_tymy" id="hraji_tymy" />
    ...
    <option <?if($ngData->hraji_tymy==0){?>selected<?}?> 
    value="0"><?=$systemTranslator['turnaje_hra_na_tymy_ne'];?></option>
    <option <?if($ngData->hraji_tymy==1){?>selected<?}?> 
    value="1"><?=$systemTranslator['turnaje_hra_na_tymy_ano'];?></option>
  </select></div>  
      ...
      <?foreach($params as $p){if(count($subParams[$p->idp]>0)){?> 
          ...
          <div class="col col-1-3">
            ...
            <select name="params[]" id="param_<?=$p->idp?>">
              ...
              <?foreach($subParams[$p->idp] as $sp){?>
                ...
                <option <?if(in_array($sp->idpv,$ngData->params)){?>selected<?}?> value="<?=$sp->idpv?>" 
                ><?=$sp->nazev?></option> 
        ...
        <?}elseif($p->typ_v_turnaji_cupu==3){ //text?> 
          ...
          <div class="col col-1-3">
            ...
            <input type="text" name="paramText_<?=$p->idp?>" id="param_<?=$p->idp?>" 
            value="<?=$ngData->paramsText[$p->idp]?>" maxlength="127" />
