<h1>Hraní - Platformy</h1>
<br>
<?if(getget('message','')=='not-found'){?><h2>Platforma neexistuje.</h2><br><?}?>
<?if(getget('message','')=='created'){?><h2>Platforma úspěšně přidána.</h2><br><?}?>
<?if(getget('message','')=='not-created'){?><h2>Platformu se nepodařilo vytvořit - musíte zadat název.</h2><br><?}?>
<?if(getget('message','')=='deleted'){?><h2>Platforma úspěšně smazána.</h2><br><?}?>
<table>
  <tr><th>#ID</th><th>Název</th><th colspan="3">Aktivní</th></tr>
  <?if(count($list)>0){
    foreach($list as $l){?>  
      <tr>
        <td width="10%"><b>#<?=$l->idgp?></b></td>              
        <td><?=$l->nazev?></td>  
        <td><?=$l->aktivni==1?'Ano':'Ne'?></td>      
        <td width="3%">
          <?/*<a title="Smazat platformu" href="<?=$l->adel? >" onclick="return confirm('Opravdu si přejete smazat tuto platformu?');"><i class="fa fa-trash-o"></i></a>*/?>          
        </td>
        <td width="3%"><a title="Editovat platformu" href="<?=$l->aedit?>"><i class="fa fa-pencil"></i></a></td>         
      </tr>              
    <?}?>
    <tr> 
...
<form method="post" action="<?=$anew?>">
  <table>
    <tr><th colspan="3">Přidat platformu (název je povinný):</th></tr>
