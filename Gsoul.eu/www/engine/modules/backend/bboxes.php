...
class BBoxes extends Module {
    ...
    public function Main() {
        ...
        if ($action == 'list') {
            $this->PageList();
        } elseif ($action == 'new-post' && in_array('boxes_changes', $this->rights->povoleneKody)) {
            $this->PageNewPost();
        } elseif ($action == 'delete' && in_array('boxes_changes', $this->rights->povoleneKody)) {
            $this->PageDeletePost();
        } elseif ($action == 'save-order' && in_array('boxes_changes', $this->rights->povoleneKody)) {
            $this->PageSaveOrder();
        } elseif ($action == 'edit-save' && in_array('boxes_changes', $this->rights->povoleneKody)) {
            $this->PageSaveEdit();
        } elseif ($action == 'edit') {
            $this->PageEdit();
        } else {
  ...
  private function PageEdit()
{
    ...
    if ($box->modul == 'pages_menu') {
        $tplSub->add('sablony', array(
            '0' => 'Jednoúrovňové menu v hlavičce',
            '4' => 'Jednoúrovňové menu v patičce',
            '3' => 'Jednoúrovňové obrázkové menu'
            /*,'1'=>'Víceúrovňové menu - otevřené pouze aktivní stránky','2'=>'Víceúrovňové menu - otevřené všechny stránky'*/
        ));
    ...
    if ($box->modul == 'gallery') {
        ...
        $tplSub->add('poradi', array(
            'nazev asc' => 'Dle názvu (A-Z)',
            'nazev desc' => 'Dle názvu (Z-A)',
            'idi asc' => 'Od nejstarších',
            'idi desc' => 'Od nejnovějších',
            'RAND ()' => 'Náhodné pořadí'
        ));
    }
    ...
    if ($box->modul == 'files') {
        ...
        $tplSub->add('poradi', array(
            'nazev asc' => 'Dle názvu (A-Z)',
            'nazev desc' => 'Dle názvu (Z-A)',
            'idf asc' => 'Od nejstarších',
            'idf desc' => 'Od nejnovějších',
            'RAND ()' => 'Náhodné pořadí'
        ));
    }
}
