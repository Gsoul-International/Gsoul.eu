...
class FNews extends Module{
  public function Main(){
    ...
    if($idn>0){
      ...
      $this->seo_title=$this->kernel->systemTranslator['obecne_novinky'];   
      $this->seo_keywords=$this->kernel->systemTranslator['obecne_novinky'];  
      $this->seo_description=$this->kernel->systemTranslator['obecne_novinky'];  
      $breadcrumb=$this->kernel->models->DBnews->returnBreadcrumb(0,true,$this->kernel->systemTranslator);   
      foreach($breadcrumb as $kb=>$vb){
          if($vb->type==0){
            $breadcrumb[$kb]->link=$this->Anchor($vb->urlPieces);
          }elseif($vb->type==(-1)){
      ...
      $counter=9;
      $news=$this->kernel->models->DBnews->getLines('idn,zobrazovat,nazev,datum,predtext,id_obrazku,zobrazovat_datum','WHERE zobrazovat=1 AND id_jazyka="'.((int)$this->kernel->active_language).'" order by datum desc LIMIT '.($page*$counter).', '.$counter);         
      $images=array();
      $images2=array();               
      $news_count=$this->kernel->models->DBnews->getOne('count(idn)','WHERE zobrazovat=1 AND id_jazyka="'.((int)$this->kernel->active_language).'"'); 
