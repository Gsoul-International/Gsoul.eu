...
class DBimages extends Model{
  ...
  public function AddSizes($filename=''){
    ...
    $paths=array();
    foreach($sizes as $ks=>$vs){$paths[$ks]=str_replace('xxxx',$vs,$filename);}  
    return $paths;
    }
  public function CreateImage($file,$category=0,$nazev='',$popis=''){
    ...
    if(!in_array($suffix,$pripony)){return false;} 
    if(filesize($file["tmp_name"])>(str_replace('B','',ini_get('upload_max_filesize'))*1048576)){return false;}
    ...
    if(move_uploaded_file($file["tmp_name"],'userfiles/images/'.$category.'/'.$idi.'/_.'.$suffix)){           
      $this->RegeneratePreviews('userfiles/images/'.$category.'/'.$idi.'/_.'.$suffix,$sizes);
  ...
  public function CreateThumb($path1='',$path2='',$w=3000,$h=3000){
    ...
    if($is==false){echo 'Image Error.';die;}
	  return true;
    } 
