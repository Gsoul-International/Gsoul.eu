...
class BPages extends Module{
  ...
  public function Main(){
    ...
    $action=getget('action','main-pages');
      ...
      elseif($action=='save-order'){$this->PageSaveOrder();}      
      elseif($action=='main-pages'){$this->PageMainPages();}   
      elseif($action=='save-main-page'){$this->SaveMainPage();}                
      elseif($action=='list-news'){$this->PageListNews();}
      elseif($action=='new-news'){$this->PageNewNews();}
      elseif($action=='new-news-post'){$this->PageNewsNewPost();}
      elseif($action=='edit-news'){$this->PageEditNews();}
      elseif($action=='save-news'){$this->PageSaveNews();}  
      elseif($action=='delete-news'){$this->PageDeleteNews();}          
      else{$this->Redirect();}    
    }
  ...
  private function PageListNews(){
    ...
    $counter=10;              
    $news=$this->kernel->models->DBnews->getLines('idn,zobrazovat,nazev,datum,zobrazovat_datum','order by datum desc LIMIT '.($page*$counter).', '.$counter);
  ...
  private function PageList(){
    ...
    $tree=$this->kernel->models->DBpages->returnTree(0,'idp,parent_id,nazev,zobrazovat');
