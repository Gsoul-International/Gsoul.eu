...
class FBoxes extends Module{
  ...
  public function box_news($box){
    $tpl=new Templater();
    $tpl->add('box',$box);
    $news=$this->kernel->models->DBnews->getLines('idn,zobrazovat,nazev,datum,predtext,id_obrazku,zobrazovat_datum','WHERE zobrazovat=1 order by datum desc LIMIT 3');
    $images=array();
    $images2=array();                   
  ...
  public function box_pages_menu($box){
    ...
    if($box->sablona==0||$box->sablona==1||$box->sablona==2||$box->sablona==3){
      if($box->sablona==3){
        $tree=$this->kernel->models->DBpages->getlines('idp,typ,odkaz,nazev,nove_okno,id_obrazku','WHERE parent_id="'.$idp.'" AND zobrazovat=1 order by poradi');
      }else{
        $tree=$this->kernel->models->DBpages->getlines('idp,typ,odkaz,nazev,nove_okno,id_obrazku','WHERE parent_id="'.$box->int_1.'" AND zobrazovat=1 order by poradi');
      } 
      if($box->sablona==3){
        $images=array();
        $images2=array();
    ...
    if($box->sablona==1||$box->sablona==2){
      ...
      $tree=$this->kernel->models->DBpages->returnTree($box->int_1,'idp,parent_id,nazev,zobrazovat,typ,odkaz,nove_okno','WHERE zobrazovat=1');      
