...
class Kernel{
  ...
  public function LoginUser($email='',$pass='',$redirectto='Frontend'){    
    ...
    if($email!=''&&$pass!=''){
      $data=$this->database->MqueryGetLine('SELECT uid,heslo,heslo_2,pocet_prihlaseni FROM users WHERE email="'.trim($email).'" limit 1');
      if(isset($data->uid)&&$data->uid>0){
        $login=false;
        if((hash('sha512',$pass)==$data->heslo)&&$data->heslo!=''&&$pass!=''){$login=true;}
        if((hash('sha512',$pass)==$data->heslo_2)&&$data->heslo_2!=''&&$pass!=''){$login=true;}
          if($login==true){
