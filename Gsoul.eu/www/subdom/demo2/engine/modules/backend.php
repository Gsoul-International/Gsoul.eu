...
class Backend extends Module{
  ...
  private function MainPage(){
    ...
    if($this->kernel->user->data->prava==1){
    	...
    	$adminsRights=$this->kernel->models->DBusers->MqueryGetLines('SELECT DISTINCT ar.povoluje_modul as povoleny_modul FROM administrators_rights as ar,administrators_rights_users as aru WHERE ar.idar=aru.idar AND aru.idu="'.((int)$this->kernel->user->data->uid).'"');
    	foreach($adminsRights as $ar){$allowedModules[$ar->povoleny_modul]=$ar->povoleny_modul;}
    	unset($adminsRights);    	
    }else{
    	$allowedModules=array('BGame','BUsers','BPages','BBoxes','BFiles','BSettings');
    }   
    $topMenu=array();
    foreach($this->kernel->modules as $km=>$vm){
      if($vm->parent_module=='Backend'){  
        if($this->kernel->user->data->prava>=$vm->user_rights){
        	if(in_array($km,$allowedModules)||$km=='BSuperAdmin'){     
		        $obj=new stdClass();
		        $obj->module=$km;
