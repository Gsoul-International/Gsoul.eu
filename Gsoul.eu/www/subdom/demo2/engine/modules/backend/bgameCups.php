...
class BGameCups extends Module{
  ...
  public function PageMain(){    
    ...
    $andGroup=array();
    $andWhere='';
    if($filter->f_hra>0){$andGroup[]=' id_hry="'.((int)$filter->f_hra).'" ';}        
    if($filter->f_zobrazeni==1){$andGroup[]=' skryty=0 ';}        
    if($filter->f_zobrazeni==2){$andGroup[]=' skryty=1 ';}    
    if($filter->f_zahajeno==1){$andGroup[]=' zahajeno=1 ';}        
    if($filter->f_zahajeno==2){$andGroup[]=' zahajeno=0 ';}    
    if($filter->f_dohrano==1){$andGroup[]=' dohrano=1 ';}        
    if($filter->f_dohrano==2){$andGroup[]=' dohrano=0 ';}
    if(count($andGroup)>0){$andWhere=' WHERE '.implode(' AND ',$andGroup).' ';}
    ...
    $games2=array('0'=>' - Nezadáno - ');$types2=array('0'=>' - Nezadáno - ');$modules2=array('0'=>' - Nezadáno - ');$games3=array('0'=>' - Nezadáno - '); 
    ...
    foreach($gaxy3 as $gx3){$games3[$gx3->idg]=$gx3->nazev;}   
    $tpl=new Templater();
    $tpl->add('list',$list);  
  ...
  public function PageEdit(){
    ...
    if(!isset($data->idc)||$data->idc<1){$this->redirect(array('module'=>$this->parent_module,'action'=>'cups','message'=>'not-found'),false);}
    ...
    $users4=array();
    foreach($users3 as $us){$users4[$us->uid]=$us;}  
    ///
    $return=new stdClass();     
    $tpl=new Templater();        
    $tpl->add('data',$data);
  ...
  public function PageCupsChangeBanner(){
    ...
    if($idc<1){$this->redirect(array('module'=>$this->parent_module,'action'=>'cups-edit','idc'=>$idc),false);}
