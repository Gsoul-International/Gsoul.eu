...
class FPaypal extends Module{   
  ...
  public function PagePayIn(){
    ...
    if($this->kernel->user->uid>0&&$this->kernel->user->data->overen_paypal_email==1&&$amountP>=1&&($currencyP=='USD')){ 
      ...
      $amount->setTotal($xcost)->setCurrency('USD')->setDetails($details);                            
      $transaction = new \PayPal\Api\Transaction();
      $transaction->setAmount($amount)->setItemList($itemList)->setDescription("Credit recharge")->setInvoiceNumber(uniqid());      
      $redirectUrls = new \PayPal\Api\RedirectUrls();
      $redirectUrls->setReturnUrl("http://demo2.gsoul.eu/paypal/paypal-return/payin-success/")->setCancelUrl("http://demo2.gsoul.eu/paypal/paypal-return/payin-failed/");
  ...
  public function PagePaypalError(){    
    ...
    $this->content=$tpl->fetch('frontend/paypalThree/error.tpl');  
    $this->execute(); 
    } 
  public function PagePaypalSuccess(){
    ...
    $tpl->add('mainpagename',$mainpagename); 
    $tpl->add('systemTranslator',$this->kernel->systemTranslator);         
    $this->content=$tpl->fetch('frontend/paypalThree/success.tpl');  
    $this->execute();     
    }  
  public function PagePaypalFailed(){
    ...
    $tpl->add('mainpagename',$mainpagename);
    $tpl->add('systemTranslator',$this->kernel->systemTranslator);          
    $this->content=$tpl->fetch('frontend/paypalThree/failed.tpl');  
    $this->execute();     
    }  
  public function PagePaypalWaiting(){
    ...
    $tpl->add('mainpagename',$mainpagename); 
    $tpl->add('systemTranslator',$this->kernel->systemTranslator);         
    $this->content=$tpl->fetch('frontend/paypalThree/waiting.tpl');  
    $this->execute();     
    } 
  public function PageMain(){    
    ...
    $tpl->add('systemTranslator',$this->kernel->systemTranslator);
    $this->content=$tpl->fetch('frontend/paypalThree/main.tpl');  
    $this->execute(); 
    } 
